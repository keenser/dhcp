    ...
    class _cdict():
        def __init__(self, value=None):
            ...
            if isinstance(value, (bytes, int)):
                self._data = _char(value)
            elif isinstance(value, str):
                self._data = _char(dictionary.get(value.upper(), None))
        def __str__(self):
...
class _ipv4vlsm(ipaddress.IPv4Network):
    def __bytes__(self):
        return bytes(_char(self.prefixlen)) + self.network_address.packed[:-int(-self.prefixlen//8)]
...
class _ipv4route(tuple):
    def __new__(cls, value):
        ...
        if isinstance(value, (tuple, list)):
            ...
            while value:
                prefixlen = _char(value[:1])
                netlen = - int(-prefixlen//8)
                net = value[1:1 + netlen] + bytes(4 - netlen)
                gate = value[1 + netlen: 1 + netlen + 4]
                routelist.append([_ipv4vlsm(net), _ipv4(gate)])
                routelist[-1][0].netmask, routelist[-1][0]._prefixlen = routelist[-1][0]._make_netmask(prefixlen)
                value = value[1 + netlen + 4:]
...
HEADER_FIELDS['htype'] = Field(default=b'\x01', location=1, fmt='!1s', data=_char)
# Hardware address length
HEADER_FIELDS['hlen'] = Field(default=b'\x06', location=2, fmt='!1s', data=_char)
# Amount of hops used via relay agents
HEADER_FIELDS['hops'] = Field(default=b'\x00', location=3, fmt='!1s', data=_char)
# Transaction Identifier, 32 bit client identifier generated by the client
HEADER_FIELDS['xid'] = Field(default=b'', location=4, fmt='!4s', data=_32bits)
...
OPTION['default_ip_time-to-live'] = Option(code=23, data=_char)
OPTION['path_mtu_aging_timeout'] = Option(code=24, data=_ipv4)
OPTION['path_mtu_table'] = Option(code=25, data=_16bits)
...
# TCP parameters
OPTION['tcp_default_ttl'] = Option(code=37, data=_char)
...
OPTION['nb_node_type'] = Option(code=46, data=_char)
...
OPTION['overload'] = Option(code=52, data=_char)
...
OPTION['auto_config'] = Option(code=116, data=_char)
...
class DHCP(object):
    ...
    def serialize(option):
        ...
        data += bytes(_char(OPTION['end'].code))
    ...
    def HandleDhcpUnknown(sefl, packet):
        pass
    ...
    def GetNextDhcpPacket(self,timeout=60):
        ...
        while data == "":
            ...
            if data != "":
                ...
                packet_type = packet.get('dhcp_message_type')
                if packet_type == 'DHCP_DISCOVER':
                    self.HandleDhcpDiscover(packet)
                elif packet_type == 'DHCP_REQUEST':
                    self.HandleDhcpRequest(packet)
                elif packet_type == 'DHCP_DECLINE':
                    self.HandleDhcpDecline(packet)
                elif packet_type == 'DHCP_RELEASE':
                    self.HandleDhcpRelease(packet)
                elif packet_type == 'DHCP_INFORM':
                    self.HandleDhcpInform(packet)
                elif packet_type == 'DHCP_OFFER':
                    self.HandleDhcpOffer(packet)
                elif packet_type == 'DHCP_ACK':
                    self.HandleDhcpAck(packet)
                elif packet_type == 'DHCP_NACK':
...
class Client(DHCP):
    def __init__(self, options):
        ...
        super().__init__(self.listen, self.client_port, self.server_port)
        self.BindToAddress()
    ...
    def SendPacket(self, options = {}):
        ...
        #opt.update(options)
        #self.SendDhcpPacketTo(self.serialize(opt), self.server, self.server_port) # v3.4
